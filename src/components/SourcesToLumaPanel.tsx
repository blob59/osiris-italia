...
export default function SourcesToLumaPanel() {
  ...
  useEffect(() => {
    ...
    fetch(url).then(async r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
      .then(data => {
        if (cancelled) return;
  ...
  function sendNews(item: NewsItem) {
    ...
    pushLumaContext({ kind: 'news', label: `Notizia: ${item.title || 'senza titolo'}`, summary: 'Notizia proveniente da una fonte OSINT/RSS della piattaforma. Il punteggio di rischio è un indicatore automatico, non una verifica indipendente del contenuto.', lat: coords?.[0], lng: coords?.[1], data: { title: item.title || null, description: item.description || null, source: item.source || null, published: item.published || null, link: item.link || null, riskScore: item.risk_score ?? null } });
  }
  function sendCamera(item: CameraItem) {
    pushLumaContext({ kind: 'generic', label: `Webcam pubblica: ${item.name || item.city || 'camera'}`, summary: 'Webcam pubblica indicizzata da OSIRIS Italia. Luma riceve solo metadati e posizione; non deve dedurre identità delle persone riprese.', lat: item.lat, lng: item.lng, data: { type: 'public-webcam', id: item.id || null, name: item.name || null, city: item.city || null, country: item.country || null, source: item.source || null, feedUrl: item.feed_url || null, externalUrl: item.external_url || null } });
  }
  function sendShip(item: ShipItem) {
    pushLumaContext({ kind: 'generic', label: `Nave: ${item.name || item.mmsi || item.id || 'AIS'}`, summary: 'Traccia AIS pubblica disponibile nella piattaforma. Posizione e telemetria possono essere incomplete, ritardate o assenti; non usarle come dato tattico in tempo reale.', lat: item.lat, lng: item.lng, data: { type: 'vessel', id: item.id ?? null, mmsi: item.mmsi ?? null, name: item.name || null, vesselType: item.type || null, speed: item.speed ?? null, heading: item.heading ?? null, observedAt: item.timestamp ?? null } });
  }
  ...
  return (
    <div className="fixed bottom-[72px] left-2 z-[350] pointer-events-auto font-mono sm:bottom-4 sm:left-4 sm:z-[10000]">
      {open ? (
        <div className="w-[min(94vw,430px)] max-h-[42dvh] sm:max-h-[68vh] rounded-xl border border-white/15 bg-[#07080d]/95 shadow-2xl backdrop-blur-xl overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2.5 border-b border-white/10">
            ...
            <button onClick={() => setOpen(false)} className="p-2 -m-1 text-white/60 hover:text-white" aria-label="Chiudi sorgenti Luma"><X className="w-5 h-5"/></button>
          </div>
          ...
          <div className="max-h-[28dvh] sm:max-h-[52vh] overflow-y-auto p-2 space-y-1.5">
            ...
            {!loading && !error && items.map((raw, i) => {
              ...
              return <button key={`${tab}-${item.id || item.mmsi || i}`} onClick={() => send(raw as any)} className="w-full text-left rounded-lg border border-white/10 bg-white/[0.03] hover:border-[#D4AF37]/35 hover:bg-[#D4AF37]/[0.05] p-2.5 transition-colors"><div className="text-[11px] text-white/85 line-clamp-2">{title || 'Elemento senza nome'}</div><div className="mt-1 text-[9px] text-white/40 line-clamp-1">{sub || 'Dati pubblici OSIRIS'}</div></button>;
            })}
          </div>
        </div>
      ) : (
        <button onClick={() => setOpen(true)} className="flex items-center gap-2 rounded-full border border-white/20 bg-[#07080d]/95 px-3 py-2.5 shadow-xl backdrop-blur hover:border-[#D4AF37]/60" aria-label="Apri sorgenti per Luma"><Sparkles className="w-4 h-4 text-[#D4AF37]"/><span className="text-[11px] font-semibold text-white">Sorgenti → Luma</span></button>
      )}
    </div>
  );
}
